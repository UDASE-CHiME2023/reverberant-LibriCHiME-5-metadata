"""
Modified from https://github.com/JorisCos/LibriMix/blob/master/scripts/create_librispeech_metadata.py

MIT License

Copyright (c) 2020 JorisCos
"""
import os
import argparse
...
def create_librispeech_dataframe(librispeech_dir, subdir, speakers_md):
    ...
    for sound_path in tqdm(sound_paths, total=len(sound_paths)):
        ...
        subset = speakers_md[speakers_md['speaker_ID'] == int(spk_id)].iat[
            0, 2]
        if TRIM:
            x, sr = sf.read(sound_path)
            _, index = librosa.effects.trim(x)
        ...
        dir_md.loc[len(dir_md)] = [spk_id, sex, subset, start, end, length, rel_path]
    return dir_md
